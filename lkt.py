...
data_loc = "/data/local/rdunn/SotC/{}/data/LKT/".format(settings.YEAR)
...
slope, upper, lower = utils.median_pairwise_slopes(timeseries.times, timeseries.data, -99.9, sigma = 1.)
...
cb=plt.colorbar(mesh, cax = cbar_ax2, orientation = 'vertical', ticks = bounds[1:-1], label = "Anomaly ("+r"$^{\circ}$"+"C)", drawedges=True)
...
# colourbar
cb=plt.colorbar(mesh, cax = plt.axes(axes[2]), orientation = 'horizontal', ticks = bounds[1:-1], label = "Anomaly ("+r"$^{\circ}$"+"C)", drawedges=True)
