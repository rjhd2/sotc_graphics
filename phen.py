...
data_loc = "/data/local/rdunn/SotC/{}/data/PHEN/".format(settings.YEAR)
...
def read_modis_ts(filename):
    ...
    return sos, eos, lai # read_modis_ts
...
def run_all_plots():
    ...
    cubelist = iris.load(os.path.join(data_loc, "For_BAMS", "MODIS.CMG.2017.SOS.EOS.MAX.Anomaly.nc"))
    ...
    LABELS = ["(a) Start of Season", "(b) End of Season"]
    ...
    for a in range(2):  
        ...
        ax = plt.subplot(1, 2, a+1, projection=cartopy.crs.NorthPolarStereo())
    ...
    sos, eos, lai = read_modis_ts(os.path.join(data_loc, "For_BAMS", "MODIS.CMG.2017.SOS.EOS.MAX.AnomalyTS.csv"))
