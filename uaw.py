...
def read_uaw_ts(filename, smooth = False, annual = False):
    ...
    return grasp, erai, cera20c, merra, jra55 # read_uaw_ts
...
def run_all_plots():
    ...
    utils.plot_ts_panel(ax, [merra, erai,jra55, cera, grasp], "-", "circulation", loc = LEGEND_LOC, ncol=2, extra_labels = [" (0.06)"," (0.06)", " (0.03)", " (0.02)", " (-0.03)"])
    ...
    utils.plot_ts_panel(ax, [merra, erai, jra55, grasp], "-", "circulation", loc = LEGEND_LOC, ncol=2, extra_labels = [" (0.17)",  " (-0.40)", " (-0.51)", " (-0.30)"])
    ...
    # # sort labelling
    ax.text(0.02, 0.87, "10"+r'$^\circ$'+"S - 10"+r'$^\circ$'+"N 50hPa", transform = ax.transAxes, fontsize = settings.LABEL_FONTSIZE)
    ...
    utils.plot_smooth_map_iris(image_loc + "p2.1_UAW_{}_anoms_era".format(settings.YEAR), cube, settings.COLOURMAP_DICT["circulation"], bounds, "Anomalies from 1981-2010 (m s"+r'$^{-1}$'+")", figtext = "(w) Upper Air (850-hPa) Winds")
