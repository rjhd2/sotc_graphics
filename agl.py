...
data_loc = "{}/{}/data/AGL/".format(settings.ROOTLOC, settings.YEAR)
...
def read_glacier(filename):
    ...
    balance = utils.Timeseries("Balance", indata[:, 0], indata[:, 2]/SCALE)
    cumul_balance = utils.Timeseries("Cumulative Balance", indata[:, 0], indata[:, 3]/SCALE)
...
def run_all_plots():
    ...
    balance, cumul_balance = read_glacier(data_loc + "mb_ref.csv")
    ...
    ax1.set_xlim([1967, int(settings.YEAR)+2])
    ax1.set_ylim([-2, 0.2])
