...
cube_list = iris.load(data_loc + "TCCO_ANO_YEAR_JANJUN_JULDEC_mean_{}.nc".format(settings.YEAR))
...
utils.plot_smooth_map_iris(image_loc + "p2.1_CMO_{}_anoms".format(settings.YEAR), cube, settings.COLOURMAP_DICT["composition"], bounds, "Anomalies from 2003-{} (%)".format(settings.YEAR[2:]), figtext = "(ag) Carbon Monoxide")
...
utils.plot_smooth_map_iris(image_loc + "CMO_{}_Jul_Dec_anoms".format(settings.YEAR), cube, settings.COLOURMAP_DICT["composition"], bounds, "Anomalies from 2003-{} (%)".format(settings.YEAR[2:]), title = "July - December {}".format(settings.YEAR))
